...
class DibiTableInfo extends DibiObject
{
	...
	public function getForeignKey($name)
	{
		$this->initForeignKeys();
		$l = strtolower($name);
		if (isset($this->foreignKeys[$l])) {
			return $this->foreignKeys[$l];

		} else {
			throw new DibiException("Table '$this->name' has no foreign key '$name'.");
		}
	}
	...
	public function getIndex($name)
	{
		$this->initIndexes();
		$l = strtolower($name);
		if (isset($this->indexes[$l])) {
			return $this->indexes[$l];

		} else {
			throw new DibiException("Table '$this->name' has no index '$name'.");
		}
	}
	...
	protected function initForeignKeys()
	{
		if ($this->foreignKeys === NULL) {
			...
			foreach ($this->driver->getForeignKeys($this->name) as $info) {
				foreach ($info['local'] as $key => $name) {
					$info['local'][$key] = $this->columns[strtolower($name)];
				}

				foreach ($this->driver->getTables() as $tbl) {
					if ($tbl['name'] == $info['table']) {
						$table = new DibiTableInfo($this->driver, $tbl);
						break;
					}
				}

				if (!$table instanceof DibiTableInfo) {
					throw new DibiException("Unknown referenced table '". $info['table'] ."'");
				}

				if ($info['foreign'] == NULL && $table->getPrimaryKey() instanceof DibiIndexInfo) {
					$info['foreign'] = $table->getPrimaryKey()->getColumns();
					
				} else {
					foreach ($info['foreign'] as $key => $name) {
						$info['foreign'][$key] = $table->getColumn($name);
					}
				}

				$name = $info['name'];
				unset($info['name'], $info['table']);
				$this->foreignKeys[strtolower($name)] = new DibiForeignKeyInfo($name, $info);
			}
		}
	}
	...
	protected function initColumns()
	{
		if ($this->columns === NULL) {
			...
			foreach ($this->driver->getColumnsMeta() as $info) {
				$this->columns[] = $this->names[strtolower($info['name'])] = new DibiColumnInfo($this->driver, $info);
			}
		}
	}
...
class DibiColumnInfo extends DibiObject
{
	...
	{
		return !$this->isNullable() && !$this->isAutoIncrement() && $this->getDefault() === NULL;
	}
