 ...
 *
 * @author     Tomáš Kraina, Roman Sklenář, Michal Hodinka
...
class DibiFirebirdDriver extends DibiObject implements IDibiDriver
{
	...
	public function begin($savepoint = NULL)
	{
		if ($savepoint) {
			...
			$result = $this->query("SAVEPOINT $savepoint");

		} else {
			if (strpos(dibi::$sql, "COMMIT") === FALSE) {
				$this->query("COMMIT;");
			}
			$this->transaction = ibase_trans($this->resource);

			if (!$this->transaction) {
				throw new DibiDriverException('Unable to handle operation - failure when opening transaction.');
			}
		}
	...
	public function commit($savepoint = NULL)
	{
		if ($savepoint) {
			if (!$this->inTransaction) {
				DibiDriverException('Unable to handle operation - no opened transaction.');
			}
			$this->query("RELEASE SAVEPOINT $savepoint ONLY");
	...
	public function rollback($savepoint = NULL)
	{
		if ($savepoint) {
			if (!$this->inTransaction) {
				DibiDriverException('Unable to handle operation - no opened transaction transaction.');
			}
			$result = $this->query("ROLLBACK TO $savepoint");
	...
	public function applyLimit(&$sql, $limit, $offset)
	{
		...
		$sql .= ' ROWS ' . (int) ($offset + 1) . ' TO ' . (int) ($offset + $limit);
	}
